...
```{r}
tree <- read.csv(file="trees91.csv",header=TRUE,sep=",",stringsAsFactors=F);
names(tree)
typeof(tree$CHBR)
```
...
![Nosema ceranae](Nosema1.jpg){ width=30% }	
...
On souhaite encore procéder à une dernière modification, en rajoutant une régression locale à travers les points de chaque facette. Pas que ça ait grand sens, sur 3 concentrations, mais bon. Au passage, si on modifie l'objet p, on a moins de lignes à copier-coller. Utiliser geom_smooth
...
![Figure 3 de Eiri et al](fig3.png){ width=50% }
