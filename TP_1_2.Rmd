...
## Imprimer dans un fichier XXX Sauvegarder dans un fichier XXXX
...
XXXXXXXXXXXXX
En fait là il aurait déjà fallut introduire les fonctions
mettre la section instructions + fonctions avant "application de fonctions"? 
XXXXXXXXXXXXX

XXXX
Petit exemples pas trop compliqués:
```{r, eval=FALSE}
...
> Faire la meme chose sur val2 XX (pour rappeler la partie indexation)
...
XXX Vous verrez plus loin une autre manière de faire des itérations avec les fonctions apply qui seront alors à priviligiez XXXX
...
XXXXX Est ce que c'est necesaire ? XXXX
...
ou la derniere valeur de la fonctiont.
...
Vous pouvez aussi appeler d'autre fonctions dans vos fonctions:
...
> Faire une fonction "paire" qui prend un nombre x, et renvoie TRUE ou FALSE si ce nombre est paire.
...
> Faire une fonction "somme_carre"" qui prend 2 nombres x et y, et renvoie la somme des carrées de ces 2 nombres.
...
> Faire une fonction "filtre_positif" qui prend un vecteur de nombres, et renvoie les valeurs possitives.

```{r ,eval=F}
filtre_positif(c(1,-2,85,-9,-2,0))
filtre_positif(c(-1,-2))
```
