...
Cela créera une nouvelle variable appelée "tree". Si vous tapez "tree" à l'invite et appuyez sur Entrée, tous les numéros stockés dans la variable seront imprimés. Essayez!
...
Étant donné un vecteur de données, une tâche commune consiste à isoler des entrées particulières ou à censurer des éléments répondant à certains critères. Ici, nous montrons comment utiliser la notation d'indexation de R pour sélectionner des éléments spécifiques dans un vecteur.

## Indexation avec des logiques

Nous donnons d'abord un exemple de la façon de sélectionner des éléments spécifiques dans un vecteur. La première étape consiste à définir un vecteur de données, et la seconde étape consiste à définir un vecteur constitué de valeurs logiques. Lorsque le vecteur de valeurs logiques est utilisé pour l'index dans le vecteur de valeurs de données, seuls les éléments correspondant aux variables évaluées sur TRUE sont renvoyés:

  > a <- c ( 1 , 2 , 3 , 4 , 5 )
 > b <- c ( VRAI , FAUX , FAUX , VRAI , FAUX )
 > a [ b ]
 [1] 1 4
 > max ( a [ b ])
 [1] 4
...
Il y a cependant d'autres moments où cette option n'est pas disponible, ou si vous voulez simplement les censurer. La fonction is.na peut être utilisée pour déterminer quels éléments ne sont pas disponibles. L'opérateur logique "not" dans R est le ! symbole. Lorsqu'ils sont utilisés avec la notation d'indexation, les éléments d'un vecteur qui sont NA peuvent être facilement supprimés:

  > a <- c ( 1 , 2 , 3 , 4 , NA )
 > is.na ( a )
 [1] FAUX FAUX FAUX FAUX VRAI
 > !  is.na ( a )
 [1] VRAI VRAI VRAI VRAI FAUX
 > a [ !  is.na ( a )]
 [1] 1 2 3 4
 > b <- a [ !  is.na ( a )]
 > b
...
Toute expression logique peut être utilisée comme un index qui ouvre un large éventail de possibilités. Par exemple, vous pouvez supprimer ou mettre en évidence des entrées correspondant à des critères spécifiques. Par exemple, vous pouvez supprimer toutes les entrées au-dessus d'une certaine valeur:
...
Pour un autre exemple, supposons que vous souhaitiez rassembler les valeurs correspondant à deux facteurs différents dans un autre vecteur:
...
Ceci est une introduction de base à certaines des commandes de graphique.
...
Ici, nous examinons certaines tâches courantes qui se présentent lors du traitement des données. Ces tâches vont de l'assemblage de différents ensembles de données en des formes plus pratiques et des façons d'appliquer des fonctions à différentes parties des ensembles de données. Les rubriques de cette section démontrent une partie de la puissance de R, mais cela peut ne pas être clair au premier abord. Les fonctions sont couramment utilisées dans une grande variété de circonstances pour un certain nombre de raisons différentes. Ces outils m'ont permis d'économiser beaucoup de temps et d'efforts dans des circonstances que je n'aurais pas prédites à l'avance.

La chose importante à noter, cependant, est que cette section est appelée " Data Management". "Il n'est pas appelé" Manipulation de données " . Les "politiciens" manipulent "les données, nous les" gérons ".

## Ajout de données

Lorsque vous avez plus d'un ensemble de données, vous pouvez les rapprocher. Vous pouvez regrouper différents ensembles de données en les ajoutant en tant que lignes ( rbind ) ou en les ajoutant en tant que colonnes ( cbind ). Le premier exemple montre comment cela est fait avec deux trames de données. Les arguments des fonctions peuvent prendre n'importe quel nombre d'objets. Nous n'en utilisons que deux ici pour simplifier la démonstration, mais des trames de données supplémentaires peuvent être ajoutées dans le même appel. Il est important de noter que lorsque vous regroupez des éléments en tant que lignes, les noms des objets dans le bloc de données doivent être identiques.

  > a <- data.frame ( one = c ( 0 , 1 , 2 ), deux = c ( "a" , "a" , "b" ))
 > b <- data.frame ( un = c ( 10 , 11 , 12 ), deux = c ( "c" , "c" , "d" ))
 > un
 un deux
 1 0 a
 2 1 a
 ...
 2 1 a 11 c
 3 2 b 12 d
 > names ( w ) = c ( "un" , "deux" , "trois" , "quatre" )
 > w
...
Les mêmes commandes fonctionnent également avec des vecteurs et des matrices et se comportent de la même manière.

  > A = matrice ( c ( 1 , 2 , 3 , 4 , 5 , 6 ), ncol = 3 , byrow = VRAI )
 > A
 [, 1] [, 2] [, 3]
 [1,] 1 2 3
 [2,] 4 5 6
 > B = matrice ( c ( 10 , 20 , 30 , 40 , 50 , 60 ), ncol = 3 , byrow = VRAI )
 > B
 [, 1] [, 2] [, 3]
 [1,] 10 20 30
 [2,] 40 50 60
...
Dans l'exemple suivant, une liste est créée avec trois éléments. Le premier est un ensemble de nombres générés aléatoirement avec une distribution normale. Le second est un ensemble de nombres générés aléatoirement avec une distribution exponentielle. Le dernier est un ensemble de facteurs. Un résumé est ensuite effectué sur chaque élément de la liste.
  ...
  > x <- liste ( a = rnorm ( 200 , moyenne = 1 , sd = 10 ),
 b = rexp (300,10,0),
 c = as.facteur (c ("a", "b", "b", "b", "c", "c")))
 > lapply ( x , résumé )
 $ a
...
### Opérations par facteurs

Une autre variante largement utilisée des fonctions d' application est la fonction tapply . La fonction tapply prendra une liste de données, généralement un vecteur, une liste de facteurs de la même liste et une fonction. Il appliquera ensuite la fonction à chaque sous-ensemble de données correspondant à chacun des facteurs.
 ...
 5 5 a
 6 50 b
 > résultat <- tapply ( val $ a , val $ b , moyenne )
 > typeof ( résultat )
 [1] "double"
 > résultat
 un B
 ...
 b
 26.66667
 > résultat <- tapply ( val $ a , val $ b , résumé )
 > typeof ( résultat )
 [1] "liste"
 > résultat
 $ a
...
Nous regardons les commandes en cours d'exécution à partir d'un fichier source. Nous incluons également une vue d'ensemble des différentes instructions utilisées pour le contrôle-flux qui détermine quel code est exécuté par l'interpréteur.
...
* répéter les déclarations
* pause et prochaines déclarations
* changement d'instruction
* instruction de scan 

Dans la section suivante, les moyens d'exécuter les commandes dans un fichier en utilisant la commande source sont donnés. Les sections restantes sont utilisées pour répertorier les différentes options de contrôle de flux disponibles dans la définition du langage R. La définition de la langue a une grande variété de fonctions de contrôle qui peuvent être trouvées en utilisant la commande help .
...
Enfin, les instructions if peuvent être chaînées pour plusieurs options. L'instruction if est considérée comme un seul bloc de code, donc plus si des instructions peuvent être ajoutées après l' autre .
...
Lorsque vous utilisez la commande sans nombre défini de lignes, la commande continue à lire les entrées du clavier jusqu'à ce qu'une ligne vierge soit entrée.
 ...
 1: yo!
 1:
 Erreur dans l'analyse (fichier, quoi, nmax, sep, dec, citation, skip, nlines, na.strings,:
