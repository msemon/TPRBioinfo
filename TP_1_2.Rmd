...
Ce document servira pour les 3 séances. Il a été largement inspiré du site suivant, dans lequel vous pourrez également trouver (en anglais) des exemples d'application des tests statistiques du type de ce que vous avez pratiqué en biostatistiques. http://www.cyclismo.org/tutorial/R/
Ainsi que de ce tutoriel http://larmarange.github.io/analyse-R/.

Vous progresserez à votre rythme et pour ceux qui iront le plus vite un autre document les attendront.

# Objectif du TP

A la fin de ces 3 séances de TP, vous devrez être capable de maitriser tous les points énumérez en ci-dessous:
...
* Maitriser les bases du language R:
    * Connaitre les différentes structures (Vecteur, tableau, marice, ...) et classes (Nombre, chaine de caractère, facteur, ...)
...
Pour commencer executer ce chunk avec la petite flèche verte
...
```{r}
heisenberg <- read.csv(file="data/seance1/simple.csv",head=TRUE,sep=",")
heisenberg
```
...
En effet ce fichier est dans le dossier "seance1" lui même dans le dossier "data". Il faut donc lui donner le chemin complet entre le dossier de tarvail et le fichier cible, donc ici "data/seance1/simple.csv".

Pour connaitre le dossier de traveil il faut executer la commande `getwd()`

```{r, echo=FALSE}
getwd()
```

Par default le dossier de travail d'un fichier .Rmd est celui qui contient le fichier. Par contre, le dossier de travail de la console est la base de votre arborescence de fichier. Exécuter directement la commande `getwd()` directement dans la console. Vous devriez avoir un résulat différent.
...
Pour connaitre les noms des colonnes de notre tableau, on peut utiliser la commande colnames ou head() pour afficher les début dut tableau.
...
La feuille de calcul d'origine se trouve à l'adresse http://cdiac.ornl.gov/ftp/ndp061a/trees91.wk1 (ou dans data_pour_exercices)
...
```{r}
tree <- read.csv(file="data_pour_exercices/trees91.csv",header=TRUE,sep=",");
names(tree)
typeof(tree$CHBR)
summary(tree)
summary(tree$CHBR)
...
Pour rajouter un élément dans un facteur, c'est **très** compliqué. Bon courage donc :
...
fac=as.factor(c(as.character(fac),"D"))
```

On peut aussi interdire à R de faire les conversions automatiques:

```{r}
tree <- read.csv(file="data_pour_exercices/trees91.csv",header=TRUE,sep=",",stringsAsFactors=F);
names(tree)
typeof(tree$CHBR)
```
...
```{r}
as.data.frame(sexsmoke)
``` 

```{r}
as.data.frame.matrix(sexsmoke)
``` 
...
## Génération automatique de vecteurs
...
## Non disponible ou valeurs manquantes (NA)
...
```{r}
tree <- read.csv(file="data_pour_exercices/trees91.csv",sep=",",head=TRUE)
tree$C <- factor(tree$C)
...
# Petits exercices et précisions pour terminer 
...
* fun.name : le nom d’un objet où est stockéee la fonction.
...
> Importer le jeu de données contenu dans "NosemaS1_Table.xls" et afficher les 5 premières lignes.
